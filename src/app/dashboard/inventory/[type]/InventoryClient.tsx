...
export default function InventoryClient({ initialItems, itemType, facilityId, locations, dropdowns }: Props) {
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:'1.2rem', paddingBottom:'.9rem', borderBottom:'1px solid rgba(201,168,76,.12)' }}>
        ...
        <button onClick={openAdd} style={{ fontFamily:'"DM Mono",monospace', fontSize:'.62rem', letterSpacing:'.1em', textTransform:'uppercase', padding:'.42rem .85rem', background:'#c9a84c', color:'#0e0c08', fontWeight:500, border:'none', borderRadius:2, cursor:'pointer' }}>
          + Add {itemType}
        </button>
      </div>
      ...
      <div style={{ display:'flex', gap:'.6rem', marginBottom:'1rem', flexWrap:'wrap' }}>
        {[
          ...
          <div key={s.l} style={{ background:'linear-gradient(135deg,#221c12,#1a160e)', border:'1px solid rgba(201,168,76,.12)', borderRadius:3, padding:'.6rem .8rem', flex:1, minWidth:90, position:'relative', overflow:'hidden' }}>
      ...
      <div style={{ display:'flex', gap:'.55rem', marginBottom:'1rem', flexWrap:'wrap', alignItems:'center' }}>
        <div style={{ position:'relative', flex:1, minWidth:180 }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder={`Search ${itemType}s…`}
            style={{ width:'100%', background:'#0e0c08', border:'1px solid rgba(201,168,76,.28)', color:'#f0ead8', padding:'.42rem .7rem .42rem 1.9rem', fontFamily:'"Crimson Pro",serif', fontSize:'.92rem', borderRadius:2, outline:'none' }} />
        </div>
        <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
          style={{ background:'#0e0c08', border:'1px solid rgba(201,168,76,.28)', color:'#f0ead8', padding:'.42rem .65rem', fontFamily:'"DM Mono",monospace', fontSize:'.63rem', borderRadius:2, outline:'none' }}>
            ...
            <thead>
              <tr>
                {['Tag ID','Name',isCostume?'Type':'Type','Period','Condition','Status','Location',''].map(h => (
                  <th key={h} style={{ fontFamily:'"DM Mono",monospace', fontSize:'.57rem', letterSpacing:'.14em', textTransform:'uppercase', color:'#8a6f30', padding:'.55rem .75rem', borderBottom:'1px solid rgba(201,168,76,.28)', background:'#0e0c08', textAlign:'left', whiteSpace:'nowrap' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
      ...
      {modal !== 'none' && (
        <div onClick={e => e.target === e.currentTarget && closeModal()}
          ...
          <div style={{ background:'linear-gradient(145deg,#221c12,#181410)', border:'1px solid #c9a84c', borderRadius:4, width:'100%', maxWidth:860, boxShadow:'0 28px 70px rgba(8,6,3,.65)', flexShrink:0 }}>
            {/* Modal header */}
              ...
              <>
                ...
                <div style={{ display:'flex', borderBottom:'1px solid rgba(201,168,76,.28)', overflowX:'auto' }}>
                  {['basic','details', ...(isCostume?['measurements']:[]), ...(isProp?['prop-details']:[]), 'photos','location','production','financial'].map(tab => (
                    <button key={tab} onClick={() => setActiveTab(tab)} style={{
                      ...
                      padding:'.5rem .9rem', cursor:'pointer', border:'none', background:'transparent',
                      borderBottom: activeTab===tab ? '2px solid #c9a84c' : '2px solid transparent',
                      color: activeTab===tab ? '#c9a84c' : '#5a5038', whiteSpace:'nowrap', transition:'all .15s'
                    }}>
                ...
                <div style={{ padding:'1.3rem' }}>
                  ...
                  {activeTab === 'basic' && (
                    <FormGrid cols={2}>
                      ...
                      <div style={{ gridColumn:'1/-1', display:'flex', gap:'1.2rem', marginTop:'.3rem' }}>
                        <label style={{ display:'flex', alignItems:'center', gap:'.4rem', cursor:'pointer', fontSize:'.88rem', color:'#9a8e72' }}>
                      ...
                      {isProp && <>
                        ...
                        <div style={{ gridColumn:'1/-1', display:'flex', gap:'1.2rem', marginTop:'.3rem' }}>
                          {[['can_be_painted','Can Be Painted'],['can_be_stood_on','Can Be Stood On'],['is_functional','Is Functional'],['can_be_controlled_remotely','Remote Control']].map(([k,l]) => (
                            <label key={k} style={{ display:'flex', alignItems:'center', gap:'.4rem', cursor:'pointer', fontSize:'.85rem', color:'#9a8e72' }}>
                  ...
                  {activeTab === 'measurements' && isCostume && (
                    <div>
                      <p style={{ fontFamily:'"DM Mono",monospace', fontSize:'.6rem', color:'#5a5038', marginBottom:'.8rem' }}>All measurements in inches unless noted</p>
                      <FormGrid cols={4}>
                  ...
                  {activeTab === 'photos' && (
                    <div>
                      <p style={{ fontFamily:'"DM Mono",monospace', fontSize:'.6rem', color:'#5a5038', marginBottom:'.8rem' }}>Upload up to 6 photos. The first photo marked primary appears on the item card.</p>
                      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(110px,1fr))', gap:'.5rem', padding:'.6rem', background:'#0a0806', border:'2px dashed rgba(201,168,76,.28)', borderRadius:3 }}>
                        {existingPhotos.map((p: any, i: number) => (
                          <div key={p.id} style={{ position:'relative', borderRadius:2, overflow:'hidden', border:`1px solid ${p.is_primary?'#c9a84c':'rgba(201,168,76,.2)'}` }}>
                            {p.is_primary && <div style={{ position:'absolute', top:2, left:2, fontFamily:'"DM Mono",monospace', fontSize:'.48rem', background:'#c9a84c', color:'#0e0c08', padding:'1px 4px', borderRadius:2, zIndex:1 }}>PRIMARY</div>}
                            {p.public_url ? <img src={p.public_url} alt="" style={{ width:'100%', height:80, objectFit:'cover', display:'block' }} /> : <div style={{ width:'100%', height:80, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1.8rem', background:'#0e0c08' }}>📷</div>}
                            <div style={{ padding:'.2rem .35rem', fontFamily:'"DM Mono",monospace', fontSize:'.52rem', color:'#9a8e72', borderTop:'1px solid rgba(201,168,76,.1)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{p.label || `Photo ${i+1}`}</div>
                            <button onClick={() => removeExistingPhoto(p.id)} style={{ position:'absolute', top:2, right:2, background:'rgba(139,26,46,.8)', border:'none', color:'#fff', width:16, height:16, borderRadius:8, cursor:'pointer', fontSize:'.6rem', display:'flex', alignItems:'center', justifyContent:'center' }}>✕</button>
                          </div>
                        ))}
                        {uploadedPhotos.map((p, i) => (
                          <div key={i} style={{ position:'relative', borderRadius:2, overflow:'hidden', border:`1px solid ${p.primary?'#c9a84c':'rgba(201,168,76,.2)'}` }}>
                            {p.primary && <div style={{ position:'absolute', top:2, left:2, fontFamily:'"DM Mono",monospace', fontSize:'.48rem', background:'#c9a84c', color:'#0e0c08', padding:'1px 4px', borderRadius:2, zIndex:1 }}>PRIMARY</div>}
                            <img src={p.previewUrl} alt="" style={{ width:'100%', height:80, objectFit:'cover', display:'block' }} />
                            <div style={{ padding:'.2rem .35rem', fontFamily:'"DM Mono",monospace', fontSize:'.52rem', color:'#9a8e72', borderTop:'1px solid rgba(201,168,76,.1)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{p.label}</div>
                            <button onClick={() => setUploadedPhotos(prev => prev.filter((_,j)=>j!==i))} style={{ position:'absolute', top:2, right:2, background:'rgba(139,26,46,.8)', border:'none', color:'#fff', width:16, height:16, borderRadius:8, cursor:'pointer', fontSize:'.6rem' }}>✕</button>
                          </div>
                        ))}
                        {(existingPhotos.length + uploadedPhotos.length) < 6 && (
                          <div onClick={() => fileInputRef.current?.click()} style={{ height:80, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1.8rem', cursor:'pointer', color:'#5a5038', border:'1px dashed rgba(201,168,76,.18)', borderRadius:2, transition:'color .15s' }}>＋</div>
                        )}
                      </div>
                      <input ref={fileInputRef} type="file" accept="image/*" multiple onChange={e => handlePhotoFiles(e.target.files)} style={{ display:'none' }} />
                      <button onClick={() => fileInputRef.current?.click()} style={{ marginTop:'.6rem', fontFamily:'"DM Mono",monospace', fontSize:'.6rem', letterSpacing:'.1em', textTransform:'uppercase', padding:'.38rem .7rem', background:'transparent', color:'#9a8e72', border:'1px solid rgba(201,168,76,.28)', borderRadius:2, cursor:'pointer' }}>📂 Browse Files</button>
                    </div>
                  )}
                  ...
                  {activeTab === 'production' && (
                    <div>
                      <div style={{ fontFamily:'"DM Mono",monospace', fontSize:'.6rem', letterSpacing:'.14em', color:'#8a6f30', textTransform:'uppercase', marginBottom:'.5rem' }}>Productions Used In</div>
                      {(form.used_in_productions || []).map((p: string, i: number) => (
                        <div key={i} style={{ display:'flex', gap:'.4rem', marginBottom:'.35rem' }}>
                          <input value={p} onChange={e => { const arr = [...(form.used_in_productions||[])]; arr[i]=e.target.value; setF('used_in_productions',arr) }}
                            style={{ flex:1, background:'#0d0a06', border:'1px solid rgba(201,168,76,.28)', color:'#f0ead8', padding:'.38rem .55rem', fontFamily:'"Crimson Pro",serif', fontSize:'.9rem', borderRadius:2, outline:'none' }} />
                          <button onClick={() => setF('used_in_productions',(form.used_in_productions||[]).filter((_:any,j:number)=>j!==i))}
                      ...
                      <button onClick={() => setF('used_in_productions',[...(form.used_in_productions||[]),''])}
                        style={{ fontFamily:'"DM Mono",monospace', fontSize:'.6rem', letterSpacing:'.1em', textTransform:'uppercase', padding:'.35rem .65rem', background:'transparent', color:'#9a8e72', border:'1px solid rgba(201,168,76,.28)', borderRadius:2, cursor:'pointer', marginTop:'.2rem' }}>
                        + Add Production
                      </button>
                      {isCostume && (
                        <FormGrid cols={2} style={{ marginTop:'1rem' }}>
                ...
                <div style={{ padding:'.85rem 1.3rem', borderTop:'1px solid rgba(201,168,76,.28)', display:'flex', justifyContent:'flex-end', gap:'.45rem', background:'rgba(14,12,8,.3)', position:'sticky', bottom:0, zIndex:2 }}>
                  ...
                  <button onClick={saveForm} disabled={saving} style={{ fontFamily:'"DM Mono",monospace', fontSize:'.62rem', letterSpacing:'.1em', textTransform:'uppercase', padding:'.42rem .85rem', background:'#c9a84c', color:'#0e0c08', fontWeight:500, border:'none', borderRadius:2, cursor:saving?'wait':'pointer', opacity:saving?.7:1 }}>
...
function ItemCard({ item, locations, onView, onEdit, onDelete }: { item: InventoryItem; locations: Location[]; onView: (i:InventoryItem)=>void; onEdit: (i:InventoryItem)=>void; onDelete: (id:string)=>void }) {
  ...
  return (
    <div onClick={() => onView(item)} style={{ background:'linear-gradient(145deg,#221c12,#1a160e)', border:'1px solid rgba(201,168,76,.18)', borderRadius:3, overflow:'hidden', cursor:'pointer', transition:'all .18s' }}>
      <div style={{ position:'relative', height:130, background:'#0a0806', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'3rem', overflow:'hidden' }}>
    ...
    <div style={{ padding:'1.3rem' }}>
      <div style={{ display:'flex', gap:'1.2rem', marginBottom:'1.2rem', alignItems:'flex-start', flexWrap:'wrap' }}>
        <div style={{ width:200, flexShrink:0 }}>
          <div style={{ width:'100%', height:170, background:'#0a0806', border:'1px solid rgba(201,168,76,.28)', borderRadius:3, display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden', fontSize:'5rem', marginBottom:'.4rem' }}>
          ...
          {photos.length > 1 && (
            <div style={{ display:'flex', gap:'.3rem', flexWrap:'wrap' }}>
              {photos.map((p,i) => (
                <div key={p.id} onClick={() => setActivePhoto(i)} style={{ width:44, height:36, background:'#0a0806', border:`1px solid ${i===activePhoto?'#c9a84c':'rgba(201,168,76,.2)'}`, borderRadius:2, overflow:'hidden', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1rem' }}>
        ...
        <div style={{ flex:1 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'.5rem' }}>
            {[
              ...
              <div key={l as string} style={{ background:'#0a0806', border:'1px solid rgba(201,168,76,.07)', borderRadius:2, padding:'.3rem .45rem' }}>
      ...
      <div style={{ display:'flex', justifyContent:'flex-end', gap:'.45rem', paddingTop:'.9rem', borderTop:'1px solid rgba(201,168,76,.12)' }}>
        <button onClick={onEdit} style={{ fontFamily:'"DM Mono",monospace', fontSize:'.62rem', letterSpacing:'.1em', textTransform:'uppercase', padding:'.42rem .85rem', background:'#c9a84c', color:'#0e0c08', fontWeight:500, border:'none', borderRadius:2, cursor:'pointer' }}>✎ Edit</button>
      </div>
    </div>
  )
}
